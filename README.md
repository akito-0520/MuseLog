...
| サービス  | 技術スタック | 役割・選定理由                                                                                                     |
| :-------- | :----------- | :----------------------------------------------------------------------------------------------------------------- |
| **Redis** | **Redis 7+** | **APIキャッシュ**。<br>DMM API レスポンスをキャッシュしてレート制限を回避。<br>女優情報の一時保存（TTL: 24時間）。 |
...
erDiagram
    ...
    reviews {
        ...
        smallint rating "1〜5の個人的評価 (nullable)"
    ...
    tags {
        ...
        datetime created_at
        datetime updated_at
        unique(user_id, name) "1ユーザー内で重複不可"
    }
...
| 項目                     | 目標値                                      |
| :----------------------- | :------------------------------------------ |
| **ページロード時間**     | 初回: 3秒以内、2回目以降: 1秒以内           |
| **API レスポンスタイム** | 95パーセンタイル: 500ms以内                 |
| **稼働率**               | 99.5%（月間ダウンタイム3.6時間以内）        |
| **同時接続数**           | 100ユーザー（初期想定）                     |
| **データバックアップ**   | 日次自動バックアップ（Supabase）、7日間保持 |
...
| **Frontend Unit Test** | Jest, React Testing Library       | 70%以上          |
...
- **開発者**: Muse Log Architecture Team
- **ライセンス**: MIT (Private Project)
